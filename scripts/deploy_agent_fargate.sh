...
# Manual deploy script for agent-service on ECS Fargate.
# Prerequisites:
...
ECR_REPOSITORY="${ECR_REPOSITORY:-guus-voice-agent-worker}"
ECS_CLUSTER="${ECS_CLUSTER:-guus-voice-agent-cluster}"
ECS_SERVICE="${ECS_SERVICE:-guus-voice-agent-worker-service}"
TASK_FAMILY="${TASK_FAMILY:-guus-voice-agent-worker-task}"
...
echo "==> Building worker image: ${ECR_URI}"
docker build -t "${ECR_URI}" "${AGENT_DIR}"
